...
--> statement-breakpoint
ALTER TABLE "documents" ADD COLUMN IF NOT EXISTS "owner_id" uuid;--> statement-breakpoint
ALTER TABLE "documents" ADD COLUMN IF NOT EXISTS "storage_bucket" text DEFAULT 'subscription-documents';--> statement-breakpoint
ALTER TABLE "documents" ADD COLUMN IF NOT EXISTS "storage_path" text;--> statement-breakpoint
ALTER TABLE "documents" ADD COLUMN IF NOT EXISTS "original_filename" text;--> statement-breakpoint
...
BEGIN
	IF NOT EXISTS (
		SELECT 1 FROM pg_constraint WHERE conname = 'user_avatars_owner_id_users_id_fk'
	) THEN
		ALTER TABLE "user_avatars" ADD CONSTRAINT "user_avatars_owner_id_users_id_fk" FOREIGN KEY ("owner_id") REFERENCES "public"."users"("id") ON DELETE cascade ON UPDATE no action;
	END IF;
END $$;
--> statement-breakpoint
CREATE UNIQUE INDEX IF NOT EXISTS "user_avatars_owner_id_idx" ON "user_avatars" USING btree ("owner_id");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "user_avatars_storage_path_idx" ON "user_avatars" USING btree ("storage_path");--> statement-breakpoint
DO $$
BEGIN
	IF NOT EXISTS (
